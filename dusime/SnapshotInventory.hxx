...
public:
  ...
  typedef std::function<void(const std::string &, const SnapshotData &)>
    fun_newset_t;
...
private:
  ...
  const std::string findUniqueName() const;

  /** Find a free file name for a snapshot */
  const std::string snapshotFileName(std::string base,
