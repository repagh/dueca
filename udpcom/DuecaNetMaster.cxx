...
const ParameterTable *DuecaNetMaster::getParameterTable()
{
  static const ParameterTable table[] = {
    ...
    { "if-address",
      new VarProbe<_ThisClass_, vstring>(&_ThisClass_::interface_address),
    ...
    { "n-logpoints",
      ...
      "and capacity." },
    { "node-list",
      new VarProbe<_ThisClass_, std::vector<int>>(&_ThisClass_::peer_nodeids),
    ...
    { "set-timing",
      new VarProbe<_ThisClass_, TimeSpec>(&_ThisClass_::time_spec),
      "Time interval" },
    ...
    { NULL, NULL,
      ...
      "broadcast, point-to-point or websocket, depending on the data url." }
  };
  return table;
}
...
bool DuecaNetMaster::complete()
{
  ...
  if (fill_unpacker) {
    fill_unpacker->initialiseStores(0, ObjectManager::single()->getNoOfNodes());
  }
...
void DuecaNetMaster::whenUp(const TimeSpec &ts)
{
  if (n_logpoints) {
    ...
    w_logtiming.reset(new ChannelWriteToken(
      getId(), NameSet("NetCommLog://dueca"), NetTimingLog::classname,
      ldata.str(), Channel::Events, Channel::OneOrMoreEntries,
    ...
    for (int ii = 0; ii < ObjectManager::single()->getNoOfNodes(); ii++) {
      ...
      if (ii) {
        ...
        log_capacity[ii] =
          new NetCapacityLog(ObjectManager::single()->getLocation());
      }
    }
  }
}
...
DuecaNetMaster::PeerMeta::PeerMeta(uint32_t nodeid, const std::string &name,
                                   uint32_t sendorder) :
  nodeid(nodeid),
...
void DuecaNetMaster::clientDecodeConfig(AmorphReStore &s, unsigned peer_id)
{
  ...
  try {
    ...
    E_NET("failed to get client config from peer with send order " << peer_id);
    throw(e);
  }
}
...
DuecaNetMaster::VettingResult
DuecaNetMaster::clientAuthorizePeer(CommPeer &peer, const TimeSpec &ts)
{
...
template <class D> D limit(D low, D val, D hig)
{
...
void DuecaNetMaster::clientPackPayload(MessageBuffer::ptr_type buffer)
{
  ...
  DEB1("pack o=" << control_size + 8 << " r=" << breg - 4
                 << " f=" << buffer->fill);
#if 0
...
void DuecaNetMaster::clientUnpackPayload(MessageBuffer::ptr_type buffer,
