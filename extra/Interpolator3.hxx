...
template <class T> class Interpolator3
{
  /** First index object. */
...
public:
  /** Constructor. */
  Interpolator3(const InterpIndex<T> &index1, const InterpIndex<T> &index2,
...
public:
  ...
  const T getValue(const InterpTable3<T, InterpIndex<T>> &table) const
  {
    return (1.0 - frac1) *
             ((1.0 - frac2) *
                ((1.0 - frac3) * table.getValue(idx1, idx2, idx3) +
           ...
           + frac1 *
               ((1.0 - frac2) *
                  ((1.0 - frac3) * table.getValue(idx1 + 1, idx2, idx3) +
                   frac3 * table.getValue(idx1 + 1, idx2, idx3 + 1)) +

                frac2 *
                  ((1.0 - frac3) * table.getValue(idx1 + 1, idx2 + 1, idx3) +
