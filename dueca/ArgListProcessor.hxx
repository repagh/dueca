...
protected:
  ...
  ArgListProcessor(const ParameterTable *table, const std::string &name,
...
public:
  ...
  bool processList(const T &specifications, ArgElement::arglist_t &processed,
                   R &reference) const;
...
*/
  ...
  bool processList(const D &specifications, const T &args,
  ...
  void printCoreCreationCall(std::ostream &os, const std::string &name,
  ...
  template <unsigned mxsize>
  bool processValue(void *m, int idx,
                    const std::vector<Dstring<mxsize>> &value) const;
...
private:
  ...
  bool processPairs(const SCM &specifications, ArgElement::arglist_t &processed,
                    SchemeObject &ref) const;
