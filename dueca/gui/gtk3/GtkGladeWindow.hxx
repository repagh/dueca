...
/** A GUI window directly from a glade interface file. Supply the
    ...
    TestObject deflt;
    mywindow.setValues(deflt, "mywidgets_%s", NULL, true);

    // .... later, after changes, read the values from the interface
    // send over a channel if applicable
    {
      DataWriter writer(w_mytoken, ts);
      mywindow.getValues(writer.data(), "mywidgets_%s", NULL, true);
    }
    @endcode
  ...
  Note for when you are transitioning to gtk4 interfaces: the GtkComboBox is
  deprecated there, replace it by a GtkDropDown.
...
public:
  ...
  */
  GObject *getObject(const char *name, bool warn = false);
...
public:
  ...
  */
   ...
   */
  template <typename T> bool loadComboText(const char *name, const T &values);
...
bool GtkGladeWindow::loadComboText(const char *name, const T &values)
{
  ...
  if (o == NULL) {
     ...
     */
    W_XTR("GtkGladeWindow::loadComboText: Could not find gtk object with id \""
          << name << "\"");
    return false;
  }
  if (!GTK_IS_COMBO_BOX(o)) {
    /* DUECA graphics.
       ...
       combo box.
    */
    W_XTR("GtkGladeWindow::loadComboText: Cannot fill options, object not a "
          "ComboBox \""
          << name << '"');
    return false;
  }
