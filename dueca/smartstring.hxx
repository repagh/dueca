...
void JSONtoDCO(const rapidjson::GenericValue<rapidjson::UTF8<>> &reader,
...
public:
  ...
  struct json_string_writer :
    public rapidjson::Writer<rapidjson::GenericStringBuffer<rapidjson::UTF8<>>>
  {
  private:
    /** Reference to the resulting output/buffer */
    std::string &result;
  ...
  public:
    ...
    virtual void force_sync() { result = doc.GetString(); }

    /** Destructor */
    ~json_string_writer() { result = doc.GetString(); }
  };
...
public:
   ...
   */
  template <class DCO> void decodexml(DCO &obj) const
  {
  ...
  /** Encode the given DCO object as XML in this string
      ...
      @param obj    DCO object
   */
  template <class DCO> void encodexml(const DCO &obj)
  {
   ...
   */
  template <class DCO> void decodejson(DCO &obj) const
  {
    CommObjectWriter cow(getclassname<DCO>(), &obj);
  ...
  /** Encode the given DCO object as JSON in this string
      ...
      @param obj    DCO object
  */
  template <class DCO> void encodejson(const DCO &obj)
  {
    CommObjectReader cor(getclassname<DCO>(), &obj);
    rapidjson::StringBuffer doc;
