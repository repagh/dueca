...
TriggerPuller::TargetData::TargetData(
  const boost::intrusive_ptr<TriggerTarget> &t, unsigned id) :
  target(t),
  id(id),
  manager(NULL)
{
...
const std::string &TriggerPuller::getTriggerName() const { return name; }

bool TriggerPuller::setManager(ActivityManager *mgr, ActivityManager *oldmgr,
...
{
  for (targetlist_type::iterator tt = targets.begin(); tt != targets.end();
       tt++) {
    if (t == tt->target.get()) {
      tt->manager = NULL;
      tt->target.reset();
...
{
  static Arena *my_arena = arena_pool.findArena(sizeof(TimingList));
...
TriggerTarget::TriggerTarget() :
  INIT_REFCOUNT_COMMA manager(NULL)
{
#if PRELOAD_REF
  intrusive_ptr_add_ref(this);
#endif

  DEB("TriggerTarget created " << reinterpret_cast<void *>(this) << " count "
                               << use_count());
}

TriggerTarget::~TriggerTarget()
{
  DEB("TriggerTarget deletion " << reinterpret_cast<void *>(this) << " count "
                                << use_count());
  for (pullers_type::iterator pp = pullers.begin(); pp != pullers.end(); pp++) {
    if (pp->puller) {
...
bool TriggerTarget::passManager(ActivityManager *mgr, ActivityManager *oldmgr)
{
  manager = mgr;
  bool res = true;
  for (pullers_type::iterator ii = pullers.begin(); ii != pullers.end(); ii++) {
    res = res && ii->puller->setManager(
                   mgr, oldmgr, boost::intrusive_ptr<TriggerTarget>(this));
  }
  return res;
}
...
bool TargetAndPuller::setManager(ActivityManager *mgr, ActivityManager *oldmgr,
                                 const boost::intrusive_ptr<TriggerTarget> &t)
{
...
void ConditionOr::trigger(const DataTimeSpec &ts, unsigned idx)
{
  if (previous_tick < ts.getValidityEnd()) {
    TriggerPuller::pull(DataTimeSpec(max(ts.getValidityStart(), previous_tick),
                                     ts.getValidityEnd()));
...
ConditionAnd::ConditionAnd() :
  ...
  previous_tick(0)
{
  DEB("ConditionAnd created " << reinterpret_cast<void *>(this) << " count "
                              << use_count());
}
...
unsigned TriggerTarget::PullerData::newSpan(const DataTimeSpec &tsext,
                                            TimeTickType previous_tick)
{
  ...
  if (tsext.getValidityStart() == time_tail->endtime &&
    ...
    if (time_tail->endtime == time_tail->ts.getValiditySpan() + previous_tick &&
        time_tail->endtime > previous_tick)
      return 1U;
    return 0U;
  }
  ...
  if (tsext.extendsOrIsAfter(time_tail->endtime)) {
    ...
    if (previous_end <= previous_tick && time_tail->endtime > previous_tick)
      return 1U;
    return 0U;
  }
...
void ConditionAnd::trigger(const DataTimeSpec &ts, unsigned idx)
{
  ...
  do {
    ...
    unsigned idxp = 0;
#endif
    for (pullers_type::iterator ii = pullers.begin(); ii != pullers.end();
         ii++) {

#ifdef DEBA
      if (ts.getValidityStart() > ii->time_head->ts.getValidityEnd() + 10000) {
        DEBA(this->getTriggerName()
             << " congestion, puller " << idxp << " range " << ii->time_head->ts
             << ' ' << ii->time_tail->ts << " demand " << idx << ' ' << ts);
      }
#endif
