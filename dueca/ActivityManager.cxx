...
bool ActivityItem::operator<(const ActivityItem &other) const
{
  ...
  if (time_spec.getValidityStart() > other.time_spec.getValidityStart())
  ...
  return (activity->getOrder() - other.activity->getOrder() +
          ActivityItem::time_weight * int(other.time_spec.getValidityStart() -
                                          time_spec.getValidityStart())) < 0;
}

const GlobalId &ActivityItem::getOwner() { return activity->getOwner(); }

const vstring &ActivityItem::getName() { return activity->getName(); }
...
void ActivityItem::operator delete(void *v) { activityitemarena.free(v); }
#endif
...
public:
  ...
  void thread(voidfunc func, void *arg, int prio, int sched_mode,
              int sched_prio)
  {
  ...
  void join(int prio) { pthread_join(despatcher_thread, NULL); }
  ...
  void priority(int sched_mode, int sched_prio, int prio)
  {
    ...
    case SCHED_FIFO:
      if (schedpar.sched_priority != sched_prio) {
        ...
        */
        W_ACT("ActivityManager " << prio << " prio requested " << sched_prio
                                 << " actual " << schedpar.sched_priority);
      }
    case SCHED_OTHER:
      break;
    default:
...
ActivityManager::ActivityManager(int level, int sched_mode, int sched_prio) :
  NamedObject(NameSet("dueca", "ActivityManager",
                      level + 1000 * ObjectManager::single()->getLocation())),
...
void ActivityManager::completeCreation()
{
  ...
  usecs_to_fraction =
    double(ActivityBit::frac_max) / (Ticker::single()->getTimeGranule() * 1e6);
  ...
  */
  ...
  if (prio == 0) {
    dummy_graphics_update = new ActivityCallback(getId(), "graphics update",
                                                 NULL, PrioritySpec(0, 0));
  }
  trigger_for_log = new ActivityCallback(getId(), "handle log requests", &cb,
                                         PrioritySpec(0, 0));
...
void ActivityManager::doActivities0()
{
  ...
  TimeTickType timeforgraphics =
    tick + prio0_ginterval - tick % prio0_ginterval;
  ...
  while (tick < timeforgraphics) {
    ...
    while (head->getNext() != tail && exittick >= tick) {
      ...
      if (tick - astarttick > 5 * prio0_maxticks) {
        /* DUECA activity.

           Warning about a specific activity that is considered too slow, please
           fix if this is not incidental. */
        W_ACT("ActivityManager 0, slow activity "
              << to_do->getName() << " by " << to_do->getOwner() << " took "
              << (tick - astarttick) * Ticker::single()->getTimeGranule()
              << "s");
      }
      astarttick = tick;
    }
  ...
  if (tick > exittick) {
    ...
    if (++prio0_overruns % 10 == 0) {
      ...
      */
      W_ACT("ActivityManager 0 had "
            << prio0_overruns << " consecutive cycles with over "
            << prio0_maxspan << "s load, scheduled queue size " << qsize);
    }
  ...
  arena->free(to_do);
  to_do = new (arena)
    ActivityItem(dummy_graphics_update, TimeSpec(SimTime::getTimeTick()));
#else
  delete to_do;
  to_do =
    new ActivityItem(dummy_graphics_update, TimeSpec(SimTime::getTimeTick()));
#endif
...
bool ActivityManager::propagateTriggers()
{
  ...
  while (triggerq.notEmpty()) {
    ...
    DEB("running atom " << reinterpret_cast<void *>(t.data().target) << " time "
                        << t.data().ts);
    t.data().propagate();
    //natoms++;
  }
                                   ...
                                   bool in_lock)
{
  ...
  if (current_log == NULL || tick >= log_end || tick < log_start ||
      (what_to_log == ActivityBit::Suspend &&
       current_log->bit_tail->type == ActivityBit::Suspend))
    return;

  current_log->appendActivityBit(
    new ActivityBit(tick - log_start, fraction, what_to_log,
                    to_do->getDescriptionId(), to_do->getTimeSpec()));
}
...
void ActivityManager::getRealTime()
{
  ...
  fraction = uint16_t(
    ((offset_usecs + usecs_per_dt) % usecs_per_dt) * usecs_to_fraction + 0.5);
}
...
void ActivityManager::logBlockingWaitOver() { doLog(ActivityBit::Continue); }
  ...
  if (qsize > EXCESSIVE_QUEUE_SIZE) {
    ...
    if ((qsize - EXCESSIVE_QUEUE_SIZE) % 100 == 0) {
      if (to_do->isEmpty()) {
        ...
        */
        W_ACT("ActivityManager " << prio << " qsize " << qsize << " working on "
                                 << to_do->getOwner() << '/' << to_do->getName()
                                 << ' ' << to_do->getTimeSpec() << " scheduled "
...
void ActivityManager::startDoActivities()
{
  ...
  running = true;
  my.thread(ActivityManager_loopDoActivities, this, prio, sched_mode,
            sched_prio);
}
...
void ActivityManager::triggerNewLog(const TimeSpec &time)
{
  ...
  current_log = new ActivityLog(
    NodeManager::single()->getThisNodeNo(), prio, req.data().start,
    fraction_to_usecs, new ActivityBit(0, 0, ActivityBit::LogStart, 0, time));
  /* DUECA activity.

     Information on the start of a new log.
  */
  I_ACT("ActivityManager " << prio << " new log starting " << req.data().start);
