...
const ParameterTable *ChannelReplicatorPeer::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    ...
    // configuration URL
    { "config-url",
      new VarProbe<_ThisClass_, std::string>(&_ThisClass_::master_url),
      "URL of the configuration connection. Must be Websocket (start with ws\n"
      "includes port, and path), e.g., \"ws://myhost:8888/config\"" },

    { "override-data-url",
      new VarProbe<_ThisClass_, vstring>(&_ThisClass_::override_data_url),
    ...
    { "master-information-channel",
      ...
      "a ReplicatorPeerAcknowledge object when the connection is "
      "established." },

    { "sync-to-master-timing",
      new VarProbe<_ThisClass_, bool>(&_ThisClass_::sync_to_master_timing),
...
// constructor
ChannelReplicatorPeer::ChannelReplicatorPeer(Entity *e, const char *part,
...
bool ChannelReplicatorPeer::setMasterInformationChannel(
  const std::string &channelname)
{
  w_masterinfo.reset(new ChannelWriteToken(getId(), NameSet(channelname),
                                           ReplicatorPeerAcknowledge::classname,
...
void ChannelReplicatorPeer::clientDecodeConfig(AmorphReStore &s)
{
  try {
    ...
    case ReplicatorConfig::AddEntry: {
      ...
      if (cmd.slave_id == peer_id) {
        ...
        DEB("Entry confirmation chn="
            << cmd.channel_id << " tmpid=" << cmd.tmp_entry_id << " front chn="
            << candidate_readers.front().first << " front tmpid="
            << candidate_readers.front().second->getReplicatorEntryId());
        ...
        /* DUECA interconnect.
           ...
           replication. */
        I_INT("making in channel " << cmd.channel_id << " reading entry "
                                   << cmd.entry_id << " permanent, temp id "
                                   << cmd.tmp_entry_id);
        candidate_readers.front().second->setReplicatorEntryId(cmd.entry_id);
    ...
    case ReplicatorConfig::RemoveEntry:
      ...
      if (cmd.slave_id == peer_id) {
        ...
        /* DUECA interconnect.
           ...
           channel. */
        I_INT("deleting writer in channel " << cmd.channel_id << " entry id "
                                            << cmd.entry_id << " origin "
                                            << cmd.slave_id);
  ...
  while (detected_entries.notEmpty()) {
    ...
    try {
      ...
      if (ww != watched[cid]->writers.end()) {
        ...
        ReplicatorConfig cf(ReplicatorConfig::AddEntry, peer_id, cid, 0U, tmpid,
                            ei->entry_label, ei->time_aspect, ei->arity,
                            ei->packingmode, ei->transportclass);
        ...
        I_INT("reporting new entry channel " << cid << " temp entry id "
                                             << tmpid);
        ...
        candidate_readers.push_back(
          make_pair(cid, std::shared_ptr<EntryReader>(new EntryReader(
                           getId(), *ei, watched[cid]->channelname))));
  ...
  while (deleted_entries.notEmpty()) {
    try {
      ...
      if (rr != watched[cid]->readers.end()) {
        ReplicatorConfig cf(ReplicatorConfig::RemoveEntry, peer_id, cid,
        ...
        while (rc != candidate_readers.end() &&
               rc->second->getEntryId() != eid && rc->first != cid)
          rc++;
        if (rc != candidate_readers.end()) {
          ...
          W_INT("reporting deletion channel "
                << cid << " tmp entry " << rc->second->getReplicatorEntryId()
                << " postponing");
          break;
        }
        else {
...
void ChannelReplicatorPeer::clientUnpackPayload(MessageBuffer::ptr_type buffer,
                                                unsigned id,
                                                ...
                                                int usecoffset)
{
  ...
  timing->second.adjustDelta(current_tick, peertick,
